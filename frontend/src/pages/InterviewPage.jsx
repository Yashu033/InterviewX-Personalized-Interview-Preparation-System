  ...
  const getQuestion = async () => {
    if (questionCount >= 8) { 
      ...
      try {
        ...
        const res = await axios.post('https://interviewx-backend-yah00.onrender.com/api/interview/finalize', {
          user_id: user.id,
          qa_history: history
        });
    ...
    try {
      const res = await axios.post('https://interviewx-backend-yah00.onrender.com/api/interview/question', { role, context: history.map(h => h.question) });
  ...
  const submitAnswer = async () => {
    ...
    try {
      const [evalRes, aiRes] = await Promise.allSettled([
        axios.post('https://interviewx-backend-yah00.onrender.com/api/interview/evaluate', { question: currentQuestion, user_answer: answer, role }),
        selectedPersonality
          ? axios.post('https://interviewx-backend-yah00.onrender.com/api/interview/ai-followup', {
